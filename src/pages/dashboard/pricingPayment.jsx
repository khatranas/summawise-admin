...
import { Box, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, IconButton, InputAdornment, Modal, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, TextField, Typography } from '@mui/material'
import React, { useEffect, useState } from 'react'
import { ToastContainer, toast } from 'react-toastify'
import EditIcon from '@mui/icons-material/Edit'
import DeleteIcon from '@mui/icons-material/Delete'
import SearchIcon from "@mui/icons-material/Search";
import { useCreatePricingMutation, useDeletePricingMutation, useUpdatePricingMutation } from '@/network/api/pricing'
...
export const PricingPayment = () => {
    ...
    const handleUpdatePricingPayment = async () => {
        try {
            await updatePricing({ id: editingPricing._id, ...editingPricing }).unwrap();
    ...
    return (
        <div style={{ padding: 10 }}>
                ...
                <Table >
                    ...
                    <TableBody className="flex flex-col w-full">
                        {filteredData && filteredData.length > 0 ? (
                            filteredData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((pricing, index) => (
                                <TableRow key={pricing._id} hover>
                                    <TableCell className="flex-1">{page * rowsPerPage + index + 1}</TableCell>
                                    <TableCell className="flex-1">{pricing._id}</TableCell>
                                    <TableCell className="flex-1">{pricing.name}</TableCell>
                                    <TableCell className="flex-1">{pricing.price.toLocaleString()} VND</TableCell>
                                    <TableCell className="flex-1">{pricing.description}</TableCell>
                                    <TableCell className="flex-1">{new Date(pricing.createdAt).toLocaleString()}</TableCell>
                                    <TableCell className="flex-1">{new Date(pricing.updatedAt).toLocaleString()}</TableCell>
                                    <TableCell>
                                        <IconButton onClick={() => handleEditPricingPayment(pricing)}>
                                            <EditIcon color="primary" />
                                        </IconButton>
                                        <IconButton onClick={() => handleOpenConfirm(pricing._id)}>
            ...
            <Dialog open={openConfirm} onClose={handleCloseConfirm}>
                ...
                <DialogContent>
                    <DialogContentText>Bạn có chắc chắn muốn xóa gói tài khoản này không?</DialogContentText>
                </DialogContent>
                <DialogActions>
                    <Button onClick={handleCloseConfirm} color="primary">Hủy</Button>
                    <Button onClick={handleDeletePricingPayment} color="error">Xóa</Button>
                </DialogActions>
            </Dialog>
            ...
            <Modal open={openCreateModal} onClose={() => setOpenCreateModal(false)}>
                <Box sx={modalStyle}>
                    ...
                    <Box sx={{ mt: 2, display: "flex", justifyContent: "flex-end" }}>
                        ...
                        <Button variant="contained" color="primary" onClick={handleCreatePricingPayment}>
                            Tạo
                        </Button>
                    </Box>
                </Box>
                    ...
                    {editingPricing && (
                        <>
                            <TextField
                                ...
                                onChange={(e) => handleInputChange(e, setEditingPricing, editingPricing)}
                                fullWidth
                                margin="normal"
                            />
                            <TextField
                            ...
                            <Box sx={{ mt: 2, display: "flex", justifyContent: "flex-end" }}>
                                ...
                                <Button variant="contained" color="primary" onClick={handleUpdatePricingPayment}>
                                    Cập nhật
                                </Button>
                            </Box>
                        </>
